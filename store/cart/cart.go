...
func (cart *BasicCart) AddProduct(p models.Product) {
	...
	cart.lines = append(cart.lines, &CartLine{Product: p, Quantity: 1})
}
...
func (cart *BasicCart) RemoveLineForProduct(id int) {
	for pos, line := range cart.lines {
		if line.Product.ID == id {
			cart.lines = append(cart.lines[:pos], cart.lines[pos+1:]...)
		}
	}
}
...
func (cart *BasicCart) GetTotal() (total float64) {
	for _, line := range cart.lines {
		total += line.Product.Price * float64(line.Quantity)
	}
	return
}
