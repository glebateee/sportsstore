package cart

import (
	"encoding/json"
	"platform/services"
	"platform/sessions"
	"sportsstore/models"
	"strings"
)

const CART_KEY string = "cart"

type sessionCart struct {
	*BasicCart
	sessions.Session
}

func (sc *sessionCart) AddProduct(p models.Product) {
	sc.BasicCart.AddProduct(p)
	sc.SaveToSession()
}

func (sc *sessionCart) RemoveLineForProduct(id int) {
	sc.BasicCart.RemoveLineForProduct(id)
	sc.SaveToSession()
}

func (sc *sessionCart) SaveToSession() {
	builder := strings.Builder{}
	json.NewEncoder(&builder).Encode(sc.lines)
	sc.Session.SetValue(CART_KEY, builder.String())
}

func (sc *sessionCart) Reset() {
	sc.lines = []*CartLine{}
	sc.SaveToSession()
}

func RegisterCartService() {
	err := services.AddScoped(func(session sessions.Session) Cart {
		lines := []*CartLine{}
		cartFromSession := session.GetValue(CART_KEY)
		if cartValString, ok := cartFromSession.(string); ok {
			json.NewDecoder(strings.NewReader(cartValString)).Decode(&lines)
		}
		return &sessionCart{
			BasicCart: &BasicCart{lines: lines},
			Session:   session,
		}
	})
	if err != nil {
		panic(err)
	}
}
