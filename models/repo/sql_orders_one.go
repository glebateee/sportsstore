...
func (repo *SqlRepository) GetOrder(id int) (order models.Order) {
	order = models.Order{Products: make([]models.ProductSelection, 0, 5)}
	...
	if row.Err() == nil {
		...
		productLines, err := repo.Commands.GetOrderLines.QueryContext(repo.Context, id)
		if err == nil {
			for productLines.Next() {
				ps := models.ProductSelection{Product: models.Product{Category: &models.Category{}}}
				err = productLines.Scan(&ps.Quantity, &ps.Product.ID, &ps.Product.Name, &ps.Product.Description,
